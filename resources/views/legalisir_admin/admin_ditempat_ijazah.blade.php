...
@section('inti_data')
    ...
    <body>
        ...
        <form method="GET" action="{{ route('legalisir_admin.admin_ditempat_ijazah_search') }}">
            <input type="text" name="search" placeholder="Cari..." value="{{ request('search') }}">
            <button type="submit">Cari</button>
        </form>
        <a href="/legalisir/admin/ditempat/ijazah">Reload</a>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($item->role_surat == 'admin')
                                    <a href="{{ url('/legalisir/admin/ditempat/ijazah/cek_legal/' . $item->id) }}" class="btn btn-warning btn-sm">Cek Data</a>
                                @else
                            ...
                            <td>
                                @if ($item->role_surat == 'manajer_sukses')
                                    <a href="{{ url('/legalisir/admin/ditempat/ijazah/download/' . $item->id) }}" class="btn btn-primary btn-sm">Unduh</a>
                                @else
        ...
        <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="uploadModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ isset($item) ? route('legalisir_admin.admin_ditempat_ijazah_resi', $item->id) : '#' }}" method="POST" id="uploadForm">
                        @csrf
        ...
        <script>
            $('#uploadModal').on('show.bs.modal', function(event) {
                ...
                modal.find('form').attr('action', '/legalisir/admin/ditempat/ijazah/no_resi/' + id);
