...
@section('inti_data')
    ...
    <body>
        <form method="GET" action="{{ route('srt_magang.manajer_search') }}">
            <input type="text" name="search" placeholder="Cari..." value="{{ request('search') }}">
            <button type="submit">Cari</button>
        </form>
        <a href="/srt_magang/manajer">Reload</a>
                ...
                <tbody>
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('srt_magang.manajer_setuju', $item->id) }}" method="POST" style="display:inline;">
                                    @csrf
