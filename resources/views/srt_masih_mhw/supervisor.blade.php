...
@section('inti_data')
    ...
    <body>
        <form method="GET" action="{{ route('srt_masih_mhw.sv_search') }}">
            <input type="text" name="search" placeholder="Cari..." value="{{ request('search') }}">
            <button type="submit">Cari</button>
        </form>
        <a href="/srt_masih_mhw/supervisor">Reload</a>
                ...
                <tbody>
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('srt_masih_mhw.sv_setuju', $item->id) }}" method="POST" style="display:inline;">
                                    @csrf
