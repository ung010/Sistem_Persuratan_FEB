...
@section('inti_data')
    ...
    <body>
        <div class="my-3 p-3 bg-body rounded shadow-sm">
            ...
            <div class="mb-3 d-grid">
                <button type="button" class="btn btn-primary" data-toggle="modal"
                    data-target="#create_sd_modal">Tambah</button>
            </div>
                ...
                <tbody>
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->email }}</td>
                            <td>
                                <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#edit_sd_modal_{{ $item->id }}">Edit</button>
                                <form onsubmit="return confirm('Yakin ingin menghapus data admin ini?')" class="d-inline"
                                    action="{{ route('supervisor_sd.delete', $item->id) }}" method="post">
                                    @csrf
            ...
            <a href="/supervisor_sd" class="btn btn-primary">Kembali</a>
            {{ $data->withQueryString()->links() }}
        </div>
        <div class="modal fade" id="create_sd_modal" tabindex="-1" aria-labelledby="create_sd_modalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('supervisor_sd.create_sd') }}" method="POST">
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="create_sd_modalLabel">Tambah Admin</h5>
        ...
        @foreach ($data as $item)
            <div class="modal fade" id="edit_sd_modal_{{ $item->id }}" tabindex="-1"
                aria-labelledby="edit_sd_modalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form action="{{ route('supervisor_sd.edit_sd', $item->id) }}" method="POST">
                            @csrf
                            <div class="modal-header">
                                <h5 class="modal-title" id="edit_sd_modalLabel">Edit Admin</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
