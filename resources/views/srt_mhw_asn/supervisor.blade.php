...
@section('inti_data')
    ...
    <head>
        <title>
            Supervisor - Surat Keterangan mahasiswa bagi anak ASN
        </title>
    </head>

    <body>
        <form method="GET" action="{{ route('srt_mhw_asn.sv_search') }}">
            <input type="text" name="search" placeholder="Cari..." value="{{ request('search') }}">
            <button type="submit">Cari</button>
        </form>
        <a href="/srt_mhw_asn/supervisor">Reload</a>
                ...
                <tbody>
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->nama_mhw }}</td>
                            <td>
                                <form action="{{ route('srt_mhw_asn.sv_setuju', $item->id) }}" method="POST" style="display:inline;">
                                    @csrf
